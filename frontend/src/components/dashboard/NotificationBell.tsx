...
import { Bell, Check, FolderPlus, Share2, Loader2 } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { fetchNotifications, markNotificationRead, markAllNotificationsRead, withFileToken, ApiNotification } from "../../lib/api";
...
export default function NotificationBell() {
  ...
  return (
    <div className="relative" ref={wrapRef}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 max-w-[calc(100vw-1.5rem)] bg-white rounded-2xl border border-slate-100 shadow-2xl z-[300] overflow-hidden">
            ...
            {loading && items.length === 0 ? (
              ...
              items.map((n) => (
                <button
                  key={n._id}
                  onClick={() => openItem(n)}
                  className={`w-full flex gap-3 px-4 py-3 text-left border-b border-slate-50 hover:bg-slate-50 transition-colors ${n.read ? "" : "bg-primary/5"}`}
                >
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${n.type === "assignment" ? "bg-indigo-50 text-indigo-500" : "bg-emerald-50 text-emerald-500"}`}>
                    {n.type === "assignment" ? <FolderPlus size={15} /> : <Share2 size={15} />}
                  </div>
