...
export default function ProcurementRFQ({ projectId, canEdit, projectInfo, onGoToPO, openRfqId, onOpenedRfq }: { projectId: string; canEdit: boolean; projectInfo?: ProjectPdfInfo; onGoToPO?: () => void; openRfqId?: string; onOpenedRfq?: () => void }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [v, r, it, secs, subs] = await Promise.all([fetchVendors(projectId), fetchRfqs(projectId), fetchProcurementItems(projectId), fetchProcurementSections(projectId), fetchSubmittals(projectId)]);
